...
public class AlmightlyStaffIDEScreen extends Screen {
    ...
    private final List<PL> parseLines = new ArrayList<>();
    ...
    @Override protected void init() {
        super.init();
        ModNetworking.sendToServer(new ModNetworking.MsgStaffReadC2S());
        awaitingRead = true;
    ...
    private void layout() {
        ...
        for (int i = 0; i < iotas.size(); i++) {
            int w = font.width(iotas.get(i).display());
    ...
    private void topBtn(GuiGraphics g, String s, int slot, int mx, int my) {
        int bx = 220 + slot * 34; g.fill(bx, 1, bx + 32, TOP_H - 1, mx >= bx && mx < bx + 32 && my >= 0 && my < TOP_H ? 0x66_4488ff : 0x44_333333);
    ...
    private void botBtn(GuiGraphics g, String s, int slot, int mx, int my) {
        ...
        g.fill(bx, by + 1, bx + 26, by + BOT_H - 1, mx >= bx && mx < bx + 26 && my >= by && my < by + BOT_H ? 0x66_4488ff : 0x44_444444);
    ...
    @Override public boolean mouseClicked(double mx, double my, int btn) {
        ...
        if (btn == 0 && my < TOP_H) {
            int s = (int) ((mx - 220) / 34);
            if (s == 0 || s == 1) { ModNetworking.sendToServer(new ModNetworking.MsgStaffPageC2S(s == 0 ? -1 : 1)); awaitingRead = true; }
            else if (s == 2) ModNetworking.sendToServer(new ModNetworking.MsgStaffCastC2S());
    ...
    @Override public boolean mouseScrolled(double mx, double my, double dX, double dY) {
        ...
        if (ig(mx, my)) { spellcasting.mouseScrolled(mx, my, dX, dY); return true; }
    ...
    private void toggleParse() {
        ...
        if (parseMode) {
            ...
            ModNetworking.sendToServer(new ModNetworking.MsgParseToCodeC2S(iotas));
        } else {
            ModNetworking.sendToServer(new ModNetworking.MsgParseToIotasC2S(parseCode));
        }
    }
    ...
    @Override public boolean keyPressed(int k, int sc, int m) {
        ...
        if (k == GLFW.GLFW_KEY_PAGE_UP || k == GLFW.GLFW_KEY_Q) { ModNetworking.sendToServer(new ModNetworking.MsgStaffPageC2S(-1)); awaitingRead = true; return true; }
        if (k == GLFW.GLFW_KEY_PAGE_DOWN || k == GLFW.GLFW_KEY_E) { ModNetworking.sendToServer(new ModNetworking.MsgStaffPageC2S(1)); awaitingRead = true; return true; }
    ...
    private void sync() { Iota w = iotas.isEmpty() ? null : iotas.size() == 1 ? iotas.get(0) : new ListIota(new ArrayList<>(iotas)); ModNetworking.sendToServer(new ModNetworking.MsgStaffWriteC2S(w)); }
