...
public final class StaffHex {
    private static final ResourceLocation PER_WORLD_TAG = ResourceLocation.fromNamespaceAndPath("hexcasting", "per_world_pattern");
    ...
    public static List<PatternCatalogEntry> loadCatalog() {
        ...
        for (ActionRegistryEntry entry : registry) {
            ...
            boolean perWorld = registry.getHolder(id).map(h -> h.is(TagKey.create(registry.key(), PER_WORLD_TAG))).orElse(false);
            if (perWorld) {
                worldIds.add(id.toString());
    ...
    public static Component inlinePatternText(PatternRef ref) {
        try {
            return new PatternIota(patternFor(ref)).display();
        } catch (RuntimeException e) {
            return null;
        }
    }

    /** 图案 iota 的类型色（不反序列化实体，仅用于目录/色块渲染）。 */
    public static int patternColor(PatternRef ref) {
        try {
            return new PatternIota(patternFor(ref)).getType().color();
        } catch (RuntimeException e) {
            return 0x888888;
        }
    }
